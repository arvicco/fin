...
describe Orders::MoneyLimit do

  it_behaves_like 'model'

  describe '#new with empty initializer' do
    subject { Orders::MoneyLimit.new }
...
#
#    vm_reserve	    d26.2	Сумма, зарезервированная под отрицательную вариационную маржу
...
#
#    is_auto_update_limit	      i1	Признак автоматической коррекции лимита на величину
#                                дохода при закачке после клиринга: 0-нет, 1-менять.
#    is_auto_update_spot_limit	i1	Признак автоматической коррекции лимитов по Спотам
#                               (на Продажу, и на Покупку) при закачке после клиринга:
#                               0-нет, 1-менять.
#    no_fut_discount	i1	Запрещение использования скидки по фьючерсам: 1-Запрет, 0-нет.
#    limits_set	      i1	Признак установки лимитов. 0 – лимиты отсутствуют
#    premium	        d26.2	Премия
#    premium_order_reserve	f	Резерв премии под заявки
  end

  describe '#new with opts' do
    subject { Orders::MoneyLimit.new :repl_id => 12,
                                     :repl_rev => 123,
                                     :client_code => '1234',
                                     :money_old => 1234,
                                     :money_amount => 12345,
